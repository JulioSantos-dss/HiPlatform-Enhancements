// ==UserScript==
    ...
    (function() {
        ...
        function injectSpyButtons() {
            ...
            // (Removida a limpeza de olhos antigos pois o olho foi transferido para o context menu)
            ...
            existingGhosts.forEach(ghost => {
                ...
                if (parentRow) {
                    ...
                    // Se o Bitrix apagou o contador nativo, mas nós temos o fake salvo, não apaga o fantasma
            ...
            rows.forEach(row => {
                ...
                if (!row.dataset.ghostListenerAdded) {
                    ...
                    row.addEventListener('click', (e) => {
                        // Ignora cliques que foram diretamente nos nossos botões
                        if (e.target.closest('.bitrix-ghost-btn')) {
                            return;
                        }

                        // Ignora se for o clique programático simulado pelo botão fantasma
                        if (row.dataset.ignoreNextClick === 'true') {
                            row.dataset.ignoreNextClick = 'false';
                            return;
                        }

                        // CLIQUE NORMAL IDENTIFICADO: Desliga o modo fantasma da UI se estiver ativo
                        if (window.bitrixGhostMode) {
                            setGhostMode(false);
                        }

                        // Limpa o contador fake DESTE CHAT e deixa o Bitrix marcar como lido
                        if (window.ghostedChats.has(dialogId)) {
                            window.ghostedChats.delete(dialogId);
                            const fake = row.querySelector('.bitrix-fake-counter');
                            if (fake) fake.remove();
                        }
                    }, true); // Use capture para interceptar cedo
                }
                ...
                if (!row.dataset.spyContextListenerAdded) {
                    ...
                    row.addEventListener('contextmenu', (e) => {
                        ...
                        // Tenta injetar o botão no menu de contexto depois que o Bitrix renderiza ele
                        let checkAttempts = 0;
                            ...
                            if (menuContainer) {
                                if (!menuContainer.querySelector('.bitrix-spy-menu-btn')) {
                                    ...
                                    spyMenuItem.addEventListener('click', (eBtn) => {
                                        ...
                                        // Esconde o menu de contexto nativo
                                        const menu = document.getElementById('im-recent-context-menu');
                                        if (menu) menu.style.display = 'none';

                                        // Mostra o modal de espiar
                                        fetchSpyMessages(window.lastRightClickedDialogId, eBtn.clientX + 15, Math.max(10, eBtn.clientY - 100));
                                    });

                                    // Insere no topo do menu de contexto
                                    menuContainer.insertBefore(spyMenuItem, menuContainer.firstChild);
                                }
                                clearInterval(checkInterval);
                            }
                            checkAttempts++;
                            if(checkAttempts > 10) clearInterval(checkInterval); // Desiste após ~500ms
                        }, 50);
                    });
                }
                // ---------------------------------------------------------
                ...
                // Tenta encontrar o contador nativo
                for (let sel of SELECTORS.COUNTERS) {
                ...
                // Lógica do FAKE COUNTER (Persistência Visual Contínua)
                if (!hasMessages && window.ghostedChats.has(dialogId)) {
                    // Bitrix apagou nativamente porque abrimos com o fantasma, recria visualmente!
                    hasMessages = true;
                    ...
                    if (!fakeCounter) {
                        ...
                        // Estiliza o fake counter exatamente como a bolinha nativa do Bitrix
                    ...
                    // Se existe o contador nativo (chegou msg nova ou reload), apaga o fake se houver
                    const fake = row.querySelector('.bitrix-fake-counter');
                    if (fake) fake.remove();
                }

                // Se tem mensagem (seja nativa ou fake), injeta os botões
                if (hasMessages && !row.querySelector('.bitrix-ghost-btn')) {
                    ...
                    ghostBtn.style.cssText = `
                        position: absolute;
                        right: 45px; /* Movido para o lugar antigo do olho */
                        top: 50%;
                        transform: translateY(-50%);
                        z-index: 99999;
                        cursor: pointer;
                    ...
                    ghostBtn.addEventListener('click', (e) => {
                        ...
                        // Ativa o visual de chatbox fantasma (CSS cuida do resto)
                        setGhostMode(true);

                        // Salva o contador atual no "Registro Fantasma" para o Fake Counter persistir
                        // Se por algum motivo for 0 (fallback), usa 1 para forçar o badge
                        window.ghostedChats.set(dialogId, currentCount > 0 ? currentCount : (window.ghostedChats.get(dialogId) || 1));

                        // Sinaliza para o listener de clique que este não é um clique normal do usuário
                        row.dataset.ignoreNextClick = 'true';

                        // Dispara o clique nativo do Bitrix (abre o chat)
                        row.click();
                    });
        ...
        // --- Modificador de Placeholder (Roda periodicamente para injetar texto de aviso) ---
        setInterval(() => {
